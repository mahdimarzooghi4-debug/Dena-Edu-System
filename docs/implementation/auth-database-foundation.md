...
- Better Auth روی `/api/auth/[...all]` به PostgreSQL از طریق Drizzle متصل شده؛ ورود موبایلی OTP با کد شش‌رقمی و عضویت self-service فقط دانش‌آموز **در کد و CI mock** اضافه شده است، ولی بدون انتخاب vendor و `DENA_SMS_ENABLED=1` هیچ پیامک واقعی ارسال نمی‌شود. `emailAndPassword` غیرفعال است؛ [سند OTP](mobile-otp-onboarding.md).
...
- phone signup فقط پس از OTP معتبر و هنگام فعال‌سازی صریح adapter ارسال مجاز است؛ roleهای مؤسسه/ارائه‌دهنده/ادمین/سازمان/خیر هرگز self-service نیستند. حالت پیش‌فرض SMS غیرفعال است و ارائه‌دهنده واقعی هنوز انتخاب نشده است.
