...
- TLS برای SMS gateway اجباری؛ فقط هنگام `DENA_DB_INTEGRATION=1` مقصد `127.0.0.1` با HTTP جهت mock CI مجاز است. آداپتور URL خراب یا دارای userinfo، query یا fragment را بدون چاپ مقدار تنظیمات رد می‌کند و `fetch` با `redirect: "error"` هر 3xx (از جمله 307/308 با امکان ارسال مجدد body حاوی OTP) را به خطای عمومی بدون بازنشر پاسخ vendor تبدیل می‌کند. قرارداد فعلی، gateway مستقیم بدون redirect است. URLهای HTTPS با مقصد آشکار loopback/localhost، IPv4-mapped loopback، 0.0.0.0 یا link-local 169.254/16 نیز در **زمان اجرا** رد می‌شوند؛ فقط mock HTTP با مقصد دقیق 127.0.0.1 در `DENA_DB_INTEGRATION=1` مجاز است. این کنترل رشتهٔ URL از DNS rebinding یا تغییر IP مقصد پس از resolve جلوگیری نمی‌کند؛ egress allowlist و کنترل vendor در استقرار واقعی گیت مستقل‌اند. **متغیر `DENA_DB_INTEGRATION` هرگز در محیط واقعی تنظیم نشود.**
