...
خروجی موفق این قرارداد داخلی باید **هم‌زمان** status `2xx`، نوع `application/json` و بدنهٔ JSON کوچک (حداکثر ۱۰۲۴ بایت) با `{"accepted":true}` بولی داشته باشد؛ `2xx` تنها، پاسخ HTML/خالی/بیش‌ازحد بزرگ یا `{"accepted":false}` به خطای عمومی بدون نمایش محتوای vendor منجر می‌شود. این تأیید پذیرش درخواست توسط درگاه است، **نه گواهی تحویل SMS به گوشی**؛ adapter پاسخ vendor را به SDK/HTTP ثابت دنا نگاشت می‌کند. این **قرارداد داخلی برای انتخاب vendor بعدی است** و به این معنی نیست که هر پیامک‌رسان ایرانی همین endpoint را ارائه می‌دهد. پس از انتخاب vendor، فقط adapter را برای API واقعی آن تنظیم و تست می‌کنیم. نمونه کد بالا ساختگی است؛ راز عملیاتی را commit نکنید.
