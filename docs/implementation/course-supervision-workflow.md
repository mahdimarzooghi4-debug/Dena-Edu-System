...
مسیر `/institute` اکنون با session و عضویت فعال مؤسسه، حداکثر ۲۰ درخواست اخیر را صرفاً برای institute scopeهای همان فرد از PostgreSQL نشان می‌دهد. join با هر سه کلید دوره/ارائه‌دهنده/مؤسسه از نمایش رابطهٔ نامرتبط جلوگیری می‌کند؛ وضعیت requested/approved/revoked و وضعیت انتشار دیده می‌شوند، اما هیچ دادهٔ هویتی دانش‌آموزی یا حق تصمیم‌گیری تازه‌ای به صفحه منتقل نشده است. برای درخواست pending فقط لینک به صف مستقل `/institute/providers` داده می‌شود؛ تأیید یا لغو، در همان API محافظت‌شده مجوزها و استقلال بررسی‌کننده را دوباره می‌سنجد. آزمون HTTP/Playwright دسترسی ناشناس، نقش دیگر، scope دیگر، تعلیق عضویت، تغییر وضعیت و ناوبری را پوشش می‌دهد.
