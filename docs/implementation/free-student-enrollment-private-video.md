...
هیچ endpoint برای درج `objectKey` یا URL arbitrary از provider/user باز نیست. `dena_private_media_assets` باید فقط توسط یک **فرایند ingest سروری مورد اعتماد** پس از upload امن، بررسی نوع فایل، scanning، transcoding و اثبات ذخیره خصوصی ساخته شود. در این CI فقط fixture ساختگی MP4 با token داخلی localhost تولید شده است. تأیید محتوای نسخه مشخص توسط مؤسسه، استناد به مجوزهای محتوا و انتشار نسخه جدید هنوز باید طراحی شود؛ گردش‌کار دوره این مرحله نباید برای انتشار نهایی محصول تلقی شود.
