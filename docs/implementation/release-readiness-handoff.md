...
## اولین تمرین واقعی چهارگزینه‌ای (صرفاً آزمایشی)

`0012_dena_course_practice` با snapshot/journal، یک سؤال ثابت برای هر course و یک پاسخ پایدار برای هر student/course ایجاد کرده است. ارائه‌دهندهٔ همان scope بعد از نظارت مؤسسه و **قبل از انتشار** می‌تواند یک سؤال چهارگزینه‌ای با پاسخ ثابت از صفحهٔ `/provider/courses/:id/practice` و API ثبت کند؛ دوباره‌نویسی/حذف و بانک سؤال یا بازبینی مستقل سؤال نداریم. دانش‌آموز enrolled و مجاز در watch تمرین را می‌بیند، یک بار پاسخ می‌دهد، نتیجهٔ درست/نادرست سروری فقط برای خودش ذخیره می‌شود. GET دانش‌آموز کلید درست را برنمی‌گرداند و GET ارائه‌دهنده، تلاش افراد یا هویت شاگرد را انتخاب نمی‌کند. دسترسی فعلی به course ویدئویی در هر GET/POST بررسی می‌شود، لغو دسترسی آزمون را از دید دانش‌آموز پنهان می‌کند؛ رکورد قبلی برای پایلوت باقی می‌ماند. با تمرین یک‌سؤالی نباید ادعای «آزمون نمره‌دار رسمی، مدرک یا اثبات یادگیری» کرد؛ محتوا به بازبینی آموزشی، توانایی گزارش/حذف دادهٔ تلاش کودکان و الزامات حفاظت اطلاعات نیاز دارد. در تست PostgreSQL/Playwright authoring در صفحهٔ ارائه‌دهنده، پاسخ یکبارهٔ دو دانش‌آموز، replay، عدم نشت key/PII و revocation بررسی می‌شود.

## بازبینی مستقل مؤسسه برای سؤال تمرینی
