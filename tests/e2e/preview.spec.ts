...
test("all six role previews are public static shells, not live panels", async ({ request }) => {
  ...
  const protectedCourseDetail = await request.get(
    "/student/courses/11111111-1111-4111-8111-111111111111", {
      maxRedirects: 0,
    },
  );
  expect(protectedCourseDetail.status()).toBe(307);
  expect(protectedCourseDetail.headers().location).toBe("/login");
  const protectedProviderPractice = await request.get(
    "/provider/courses/11111111-1111-4111-8111-111111111111/practice", {
      maxRedirects: 0,
    },
  );
  expect(protectedProviderPractice.status()).toBe(307);
  expect(protectedProviderPractice.headers().location).toBe("/login");
  ...
  const protectedStudentPrivacy = await request.get("/student/privacy", {
    maxRedirects: 0,
  });
  expect(protectedStudentPrivacy.status()).toBe(307);
  expect(protectedStudentPrivacy.headers().location).toBe("/login");
  ...
  const protectedInstituteHome = await request.get("/institute", { maxRedirects: 0 });
  expect(protectedInstituteHome.status()).toBe(307);
  expect(protectedInstituteHome.headers().location).toBe("/login");
  const protectedPracticeReview = await request.get(
    "/institute/courses/11111111-1111-4111-8111-111111111111/practice", {
      maxRedirects: 0,
    },
  );
  expect(protectedPracticeReview.status()).toBe(307);
  expect(protectedPracticeReview.headers().location).toBe("/login");
  const protectedAdminHome = await request.get("/admin", { maxRedirects: 0 });
  expect(protectedAdminHome.status()).toBe(307);
  expect(protectedAdminHome.headers().location).toBe("/login");
  const protectedOrganizationHome = await request.get("/organization", {
    maxRedirects: 0,
  });
