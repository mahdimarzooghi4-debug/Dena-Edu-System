...
const server = createServer(async (req, res) => {
  ...
  // CI-only corruption path: even the private origin must NOT be trusted
  // to supply unsigned or modified claims.
  const corruptMatch = new RegExp(`^/__test__/corrupt/(${uuid})import { createHash, createHmac } from "node:crypto";
import { createServer } from "node:http";
...
const server = createServer(async (req, res) => {
  ...
  const inspectionMatch = new RegExp(`^/inspection/(${uuid})import { createHash, createHmac } from "node:crypto";
import { createServer } from "node:http";
